...
// Resolves a "mov/lea reg, [rip+disp32]" style reference: `prefix` are the fixed
// opcode/modrm bytes leading up to the 4-byte displacement, `suffix` are fixed
// bytes that follow it (used to disambiguate the match). Returns the absolute
// address the instruction points to, or 0 if the pattern wasn't found.
...
// ---------------------------------------------------------------------------
// Schema system: CS2 keeps a live reflection table of every network class and
// its field offsets (CSchemaSystem). Walking it at startup means the class
// field offsets above stay correct across game updates without a recompile,
// same as the dwXxx globals below.
// ---------------------------------------------------------------------------

// Walks a CUtlTSHash<SchemaClassInfoData, 256> bucket table, collecting every
// live element via both the committed bucket lists and the memory pool's
// uncommitted free-block list (CS2's hash table can have entries in either,
// depending on when they were registered). Loop counters guard against
// following a corrupted pointer chain into an infinite loop.
static std::vector<uintptr_t> WalkClassBindings(uintptr_t hashAddr) {
	...
	uintptr_t blobPtr = mem.Read<uint64_t>(hashAddr + 0x20); // entry_mem.free_blocks.head.next
...
// Finds the CSchemaSystem singleton via a signature scan of schemasystem.dll.
...
// Re-resolves every class field offset HackGame() depends on. Leaves the
// compiled-in fallback (schemas::) in place for any field a lookup misses.
